...
class Solution {
  public:
    ...
    void bfs(struct Node *root, map<int, Node*>&parents){
        queue<Node*>q;
        q.push(root);
        parents[root->data]=nullptr;
        while(!q.empty()){
            int sz=q.size();
            while(sz--){
                auto curr=q.front();
    ...
    vector<int> Ancestors(struct Node *root, int target) {
        ...
        vector<int>res;
        while(true){
            if(parents[target]==nullptr) break;
            res.push_back(parents[target]->data);
            target=parents[target]->data;
        }
        return res;
    }
};
